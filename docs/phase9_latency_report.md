...
- **专项发现(质量+延迟双重相关)**:15 轮中 5 轮(33%)命中
  ...
  整段作废改发模板。
- 查 `guard_detail`(全天真实流量 12 例)呈现**明确误杀模式**:
  `money claim <N> not in candidate price/original_price/derived set` 中的 N
  正是**用户自己输入的预算数字**(人均150→150、200元→200、500预算→500);
  LLM 在回复中复述用户预算即被判"编造价格"。极端例:`团购券可以开发票吗?`
...
| 7 | 事实校验豁免"用户输入中出现过的金额数字" | §4:12 例 guard_detail 中 ≥8 例是该误杀模式 | 每次省 6–8s 浪费流式 + 回答质量;触发率 33%→预期 <5% |
