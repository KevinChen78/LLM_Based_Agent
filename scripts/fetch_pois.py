...
def fetch_page(key, city, typecode, page):
    """Return (raw_dict, from_cache). Raises on transport/HTTP failure."""
    path = cache_path(city, typecode, page)
    if os.path.exists(path):
        with open(path, encoding="utf-8") as f:
            return json.load(f), True
    qs = urllib.parse.urlencode({
        "key": key, "types": typecode, "city": city, "citylimit": "true",
...
def main():
    ...
    for city in CITIES:
        ...
        for typecode in TYPES:
            ...
            while True:
                ...
                print(f"  {city}/{typecode} p{page}: +{len(page_pois)}"
    ...
    out = {
        "source": "amap /v3/place/text (personal demo use only)",
        "cities": CITIES,
        "types": TYPES,
        "count": len(pois),
